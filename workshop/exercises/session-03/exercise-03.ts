...
//
// The following exercise will explore how we can utilize some of the
// experimental modules being developed for Effect to introduce a persistence
// layer into our request resolvers.
//
// Your goal will be to add the requisite code to the program below to ensure
// that the following requirements are met:
//
//   - Utilize a `NodeKeyValueStore` (from `@effect/platform-node`) to persist
//     the results of requests between program invocations
//
// Hints:
//   - You probably want to make use of the `Persistence` and `RequestResolver`
//     modules from `@effect/experimental`
...
const makePokemonRepo = Effect.gen(function*(_) {
  ...
  const resolver = RequestResolver.makeBatched((
    ...
    Effect.gen(function*(_) {
      yield* _(Console.log(`Executing batch of ${requests.length} requests...`))
  ...
  const getById = (id: number) => Effect.request(new GetPokemonById({ id }), resolver)
