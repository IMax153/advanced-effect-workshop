...
const makePokemonRepo = Effect.gen(function*(_) {
  ...
  const persisted = yield* _(RRX.persisted(resolver, "GetPokemonById"))

  const getById = (id: number) => Effect.request(new GetPokemonById({ id }), persisted)
...
>() {
  static readonly Live = Layer.scoped(this, makePokemonRepo).pipe(
    Layer.provide(Persistence.layerResultKeyValueStore),
